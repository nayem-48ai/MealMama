document.addEventListener('DOMContentLoaded', () => {
    ...
    // Using a more specific key for versioning
    const aplication_data_key = 'messData_v5_final';
    let allData = JSON.parse(localStorage.getItem(aplication_data_key)) || {};
    ...
    function saveData() {
        localStorage.setItem(aplication_data_key, JSON.stringify(allData));
    }

    // --- RENDER FUNCTIONS (Unchanged from the previous bug-fixed version) ---
    ...
    // --- EVENT HANDLERS ---
    // All previous handlers (addMember, addExpense, etc.) are unchanged.
    document.getElementById('printBtn').addEventListener('click', () => {
        window.print();
    });

    // --- NEW: Reset All Data Handler ---
    document.getElementById('resetAllDataBtn').addEventListener('click', () => {
        const confirmation = confirm(
            "সতর্কবার্তা: আপনি কি নিশ্চিতভাবে সমস্ত ডেটা মুছে ফেলতে চান?\n\n" +
            "এই কাজটি ফিরিয়ে আনা যাবে না এবং আপনার সকল মাসের হিসাব স্থায়ীভাবে মুছে যাবে।"
        );

        if (confirmation) {
            // Clear the data from local storage
            localStorage.removeItem(aplication_data_key);
    ...
    // --- Daily Meal Modal Logic (Unchanged) ---
    // ...
    ...
    (function fullScript() {
        ...
        document.getElementById('addExpenseForm').addEventListener('submit', e => { e.preventDefault(); const data = getCurrentMonthData(); const date = document.getElementById('expenseDate').value; const category = document.getElementById('expenseCategory').value.trim(); const amount = parseInt(document.getElementById('expenseAmount').value); if (date && category && !isNaN(amount) && amount > 0) { data.expenses.push({ date, category, amount }); e.target.reset(); renderAll(); } });
        ...
        const memberNameInput = document.getElementById('memberNameInput'); const nameSuggestions = document.getElementById('nameSuggestions'); memberNameInput.addEventListener('input', () => { const data = getCurrentMonthData(); const input = memberNameInput.value.toLowerCase(); nameSuggestions.innerHTML = ''; if (!input) return; const filtered = data.members.filter(m => m.name.toLowerCase().includes(input)); filtered.forEach(member => { const div = document.createElement('div'); div.textContent = member.name; div.addEventListener('click', () => { memberNameInput.value = member.name; nameSuggestions.innerHTML = ''; }); nameSuggestions.appendChild(div); }); }); document.addEventListener('click', e => { if (e.target.id !== 'memberNameInput') nameSuggestions.innerHTML = ''; });
        const modal = document.getElementById('dailyMealModal'); const showBtn = document.getElementById('showDailyMealModalBtn'); const closeBtn = modal.querySelector('.close-btn'); const saveBtn = document.getElementById('saveDailyMealsBtn'); const dailyMealDate = document.getElementById('dailyMealDate'); const applyBulkMealBtn = document.getElementById('applyBulkMealBtn'); const clearDailyMealsBtn = document.getElementById('clearDailyMealsBtn');
        ...
        showBtn.onclick = () => { dailyMealDate.valueAsDate = new Date(); populateDailyMealModal(); modal.style.display = 'block'; }; closeBtn.onclick = () => modal.style.display = 'none'; window.onclick = (e) => { if (e.target == modal) modal.style.display = 'none'; }; dailyMealDate.onchange = populateDailyMealModal;
        ...
        clearDailyMealsBtn.onclick = () => { const date = dailyMealDate.value; if (!date) { alert('অনুগ্রহ করে একটি তারিখ নির্বাচন করুন।'); return; } if (confirm(`আপনি কি নিশ্চিতভাবে ${date} তারিখের সমস্ত মিলের হিসাব মুছে ফেলতে চান? এটি ফিরিয়ে আনা যাবে না।`)) { const data = getCurrentMonthData(); data.members.forEach(member => { if (data.meals[member.name] && data.meals[member.name][date]) { delete data.meals[member.name][date]; } }); modal.style.display = 'none'; renderAll(); } };
    })();
